// FinePrint v2 engine constants — NYC Local Law 97.
//
// REMADE for this hackathon and re-derived from the primary sources in
// docs/references/ (rcny_103_14.pdf, ll97_emissions.pdf). Every value here is
// transcribed verbatim from the codified rule 1 RCNY §103-14 and Admin Code
// §28-320. The engine is the single source of every number FinePrint shows;
...
// Penalty: $268 per metric ton CO2e per year over the cap. Admin Code §28-320.6;
// 1 RCNY §103-14(h). Held in integer cents internally to avoid float drift.
export const PENALTY_CENTS_PER_TCO2E = 26_800;
export const PENALTY_USD_PER_TON = 268;

// 1 therm = 100 kBtu.
export const KBTU_PER_THERM = 100;

// Fuel GHG coefficients (tCO2e per unit; electricity per kWh, fuels per kBtu).
// Per compliance period — the electricity factor falls as the grid greens.
// 1 RCNY §103-14; DOB §28-320.3.1.1.
export const FUEL_COEFFICIENTS: Record<
  Period,
  {
...
// Per-ESPM-property-type GHG emission limits (tCO2e/sf), verbatim from
// 1 RCNY §103-14. f2024 = 2024-2029, f2030 = 2030-2034, f2035 = 2035-2039.
interface EspmLimit { espmType: string; f2024: number; f2030: number; f2035: number }
const ESPM_LIMITS: EspmLimit[] = [
  ...
  { espmType: 'Multifamily Housing', f2024: 0.00675, f2030: 0.00334664, f2035: 0.002692183 },
  ...
  { espmType: 'Parking', f2024: 0.00426, f2030: 0.000214421, f2035: 0.000104943 },
...
export const ESPM_FACTORS_TCO2E_PER_SQFT: Record<Period, Record<string, number>> = {
...
// Resolve an LL84/free-text property label to its ESPM key (alias + trim).
export function resolveEspmType(label: string): string | undefined {
  const trimmed = label.trim();
  if (ESPM_FACTORS_TCO2E_PER_SQFT['2024-2029'][trimmed] !== undefined) return trimmed;
  const aliased = PROPERTY_TYPE_ALIASES[trimmed];
  if (aliased && ESPM_FACTORS_TCO2E_PER_SQFT['2024-2029'][aliased] !== undefined) return aliased;
  return undefined;
}

export function espmFactor(label: string, period: Period): number | undefined {
  const key = resolveEspmType(label);
  return key === undefined ? undefined : ESPM_FACTORS_TCO2E_PER_SQFT[period][key];
}
