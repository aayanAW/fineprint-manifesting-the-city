import type { RebateProgram } from '@/types/advise';

// Verified 2026 incentive catalog (asOf 2026-06). Sourced via research workflow against
// Con Edison / NYSERDA / IRS primary sources. Figures are program-year specific — re-verify before relying.
export const REBATES: RebateProgram[] = [
  {
    ...
    amount: 'Prescriptive full-building-load space-heating electrification (ASHP): $5,000 per dwelling unit. Prescriptive domestic hot water (HPWH): $1,000 per dwelling unit. Custom space heating or DHW: $200/MMBtu (full load); phased-load electrification $70/MMBtu. Capped at $1,000,000 per project OR 50% of project costs, whichever is less.',
  ...
  },
  {
    ...
    appliesToMultifamily: true,
    incomeEligibleBonus: true,
    amount: 'Advanced thermostats $0.16/kWh (market) / $0.18/kWh (affordable); prescriptive VFDs $0.19-$0.29/kWh; custom VFDs, booster-pump VFDs, EC motors, rooftop exhaust-fan motors $0.35/kWh (market) / $0.50/kWh (affordable); elevator modernization $0.25-$0.35/kWh; building automation system controls (secondary steam/oil savings) $40/MMBtu; boiler advanced controls (EMS) $1,500-$9,000 by unit count. Market-rate capped at 70% of project cost; affordable capped at 85%.',
    amountNumericMaxUSD: null,
    status: 'active',
    sunsetDate: '2026-11-01',
    url: 'https://www.coned.com/-/media/files/coned/documents/save-energy-money/rebates-incentives-tax-credits/rebates-incentives-for-multifamily-customers/program-incentives.pdf',
  ...
  },
  {
    ...
    appliesToMultifamily: false,
    incomeEligibleBonus: true,
    amount: 'GSHP full-load whole-building (all units in a 1-4 unit building): up to $30,000 (or 70% of project cost, whichever lower); up to $40,000 in a Disadvantaged Community (or 85% of project cost). Other utilities use different per-unit GSHP tables.',
  ...
  },
  {
    ...
    appliesToMultifamily: false,
    incomeEligibleBonus: false,
    amount: '$1,000 per HPWH unit in Con Edison territory (midstream/retail pass-through). Other utilities (Central Hudson, National Grid, NYSEG, RG&E) pay $1,250 per unit. GSHP desuperheater (Category 5b) is a separate $100/unit.',
  ...
  },
  {
    ...
    appliesToMultifamily: true,
    incomeEligibleBonus: false,
    amount: 'Tax DEDUCTION (not a rebate/credit). Tax years beginning 2025: $2.90-$5.81/sq ft with prevailing-wage & apprenticeship (PWA), $0.58-$1.16/sq ft without. 2026 (inflation-adjusted, Rev. Proc. 2025-32): up to ~$5.94/sq ft with PWA / ~$1.19/sq ft without — but only for property whose construction begins on/before 2026-06-30. Requires ≥25% modeled whole-building energy-cost reduction vs ASHRAE 90.1, scaling to 50%+ for max.',
    amountNumericMaxUSD: null,
    cashEligible: false,
    status: 'expiring',
  ...
  // IRC §45L (New Energy Efficient Home Credit) intentionally omitted: it applies only to NEW
  // construction / substantial reconstruction, so it is never claimable on an existing-building LL97
  // retrofit. Including it let a phantom $5,000/unit "cash" rebate cut net cost on the demo path.
  {
    ...
    appliesToMultifamily: true,
    incomeEligibleBonus: true,
    amount: 'Multifamily prescriptive incentive commonly cited around $5,000 per dwelling unit for full-electrification heat-pump installs; commercial full-electrification ~$120/MMBtu of displaced fossil load. Program cap 50% of project cost or $1,000,000 per account per year.',
    amountNumericMaxUSD: 5000,
    status: 'active',
    sunsetDate: null,
